...
automatically into the host machine when it powers up (as it was an
EPROM-based program).
...
- Convenient PicoRAM configuration: PicoRAM has one universal firmware
  ...
  configured to configure PicoRAM for the host machine, but no
...
- Four user memory banks: quadruples the machine's memory; the
  currently active memory bank can be selected with the `NEXT/PREV` button. 
...
PicoRAM OLED display (if not turned off) shows the currently loaded RAM file,
the machine type, and the current bank number:
...
file name creation, confirm and cancel, and so on. It will be obvious
(i.e., intuitive) how to use them.
...
the hex interval `0x000` - `0xFFF`) to properly decode the UI button
presses. The analog threshold values for the different buttons are
...
supplied init files don't work for your machine? Read further. 
...
have the option to write the acquired threshold values out to an
...
invoked by hold down any button upon start-up / reset of
PicoRAM. Examples for proper init files can be found
[here](software/).
...
256 bytes.  Users could upgrade the machine to 512 bytes by plugging
in two more 2112 SRAMs into IC16 and IC17. PicoRAM connects to IC14
and IC17 and can emulate 512 bytes of memory in the address range
`0x0000 - 0x01ff`.
...
For this mode, the 16V8 GAL chip `U10` **must** be installed. Here are
[the (optional) GAL source](src/et3400_decoder/etgal.PLD) and [the
required JED firmware file](firmware/v1.0/et3400_decoder_16v8_gal.jed)
(i.e., for programming a 16V8 GAL with a standard TL-866 EPROM
programmer and MiniPro).
...
in the early 1980s; apparently, the machine was at least being
manufactured until 1999 (my accompanying textbook has a "16th
printing, 1999" data stamp). It is equipped with 2 2114 SRAM chips (1
KB of RAM):
...
The Philips MasterLab MC6400 is a INS8070 (SC/MP III)-based CPU trainer
released in ~1984. It is equipped with 2 2114 SRAM chips (1 KB of RAM):
...
On the MPF-1(B), PicoRAM should be plugged ino the U8 6116 socket.
For the MPF-1P, the U5 6116 socket should be used. 

The `HALT` pin of the `J3` header needs to be connected to pin 37 on
the MPF's primary (top-left) extension header via a jumper wire.
Double check `JP1` as well; for the Microprofessor, it needs to be
set to R (for `CE`, pin 18).
...
latches are used to multiplex the 11-bit address bus. The (up to)
...
development log can be found on the [Hackaday project
